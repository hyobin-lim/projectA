...
## `2026-02-04` 추가 디버깅 및 구현 노트

### JavaScript `alert()`의 렌더링 차단(Rendering Blocking) 현상

*   **문제 현상:** JavaScript에서 특정 요소에 CSS 클래스를 추가하여 애니메이션을 유발한 직후 `alert()`를 호출하면, 브라우저가 화면을 다시 그리기(repaint) 전에 스크립트 실행이 멈춥니다. 이로 인해 `alert()` 창이 닫힐 때까지 애니메이션이 시작조차 하지 않는 문제가 발생했습니다. (검색 버튼 애니메이션이 동작하지 않던 원인)
*   **해결책:** `alert()` 호출을 `setTimeout`으로 감싸 아주 짧은 시간(예: 20ms)이라도 지연시킵니다. 이는 브라우저에게 제어권을 잠시 넘겨주어 CSS 변경사항(애니메이션 시작)을 렌더링할 시간을 확보하게 한 후 `alert()`를 실행하는 효과적인 해결 방법입니다.
    ```javascript
    // 예시: 애니메이션 클래스 추가
    element.classList.add('animating');

    // alert를 바로 호출하는 대신, setTimeout으로 지연
    setTimeout(() => {
      alert("애니메이션이 끝난 후 또는 시작된 후에 메시지가 보입니다.");
    }, 650); // 애니메이션 길이에 맞추거나, 렌더링 기회만 줄 경우 20ms
    ```

### SMIL vs. CSS Keyframe 애니메이션

*   **경과:** SVG 애니메이션 구현 시, 초기에는 SMIL(`<animate>` 태그)을 사용했으나 디버깅 과정에서 CSS Keyframe 방식으로 전환했다가, 최종적으로 렌더링 차단 문제를 해결한 후 다시 SMIL 방식으로 복귀했습니다.
*   **교훈:**
    *   **SMIL:** SVG 표준의 일부로, SVG와 애니메이션을 하나의 단위로 묶어 관리하기에 용이합니다. 이번 사례처럼 렌더링 차단 같은 외부 요인만 없다면 간단한 아이콘 애니메이션에 매우 적합하고 효율적입니다.
    *   **CSS Keyframes:** SMIL이 예상대로 동작하지 않을 때 시도해볼 수 있는 훌륭하고 안정적인 대안입니다. 브라우저 호환성이 높고, `transform`과 같은 속성을 다루는 데 매우 강력합니다.

### `[data-aos]` 속성과 자식 요소의 상호작용
...
*   **해결책:** 클릭이 되어야 하는 특정 자식 요소에 대해 `pointer-events: auto;` CSS 규칙을 명시적으로 추가하여 부모로부터 상속된 `pointer-events: none;` 스타일을 덮어써야 합니다. (`main_sec03` 내부의 모든 컨트롤 및 링크에 이 규칙을 적용하여 해결했습니다.)

### 파일 시스템 도구 관련 노트 (`read_file`)

*   **문제 현상:** `aos.css`와 같이 특정 CSS 파일을 `read_file` 도구로 읽을 때, 파일의 전체 내용이 아닌 일부(예: 2줄)만 반환되는 문제가 지속적으로 발생했습니다. 이로 인해 해당 파일을 포함하는 CSS 번들 생성 시 불완전한 파일이 만들어져 웹사이트 스타일이 깨지는 심각한 오류가 발생했습니다.
*   **교훈:** 파일 번들링과 같이 파일의 전체 내용을 정확히 읽어야 하는 작업 수행 시, `read_file` 도구의 반환 값을 100% 신뢰하기보다는, 반환된 내용의 길이와 원본 파일의 길이를 비교하거나, 다른 방법을 통해 파일 내용을 교차 검증하는 절차가 필요합니다.
*   **해결책:** 문제가 발생하는 파일(`aos.css`)의 경우, 신뢰할 수 있는 외부 CDN 링크를 `web_fetch` 도구로 가져와 그 내용을 사용하는 방식으로 문제를 우회했습니다.